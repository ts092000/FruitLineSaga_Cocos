    ...
    execute: function () {
      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property({
        ...
        start() {
          ...
          this.levelBar.setValueBar(0); // this.gameCenter.startMatch(() => {
        ...
        initGame() {
          //Set info turn
          this.turn.prev = _extends({}, this.turn);
        ...
        */
        ...
        onTouchStartBoard(event) {
          ...
          var currentCellComponent = this.currentFruitCell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
          }), Cell) : Cell);
          var newCoor = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
          }), Cell) : Cell).getCoordinates();
          var oldCoor = currentCellComponent.getCoordinates();
          var path = this.matrixManager.getPath(newCoor, oldCoor);
          var type = currentCellComponent.getTypeFruit();
          ...
          var index = newCoor.row * (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
        ...
        spawnCellFruit(type) {
          var cell = this.gameView.generateCells(this.matrixManager.arrayIndex, type);
          var coor = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
          }), Cell) : Cell).getCoordinates();
          var index = coor.row * (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          }), Constants) : Constants).BOARD_SIZE + coor.col;
          this.matrixManager.fillIndex(index);

          if (type === undefined) {
            var _type = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
              error: Error()
            }), Cell) : Cell).getTypeFruit();

            this.matrixManager.fillCell(index, _type);
        ...
        moveFruit(path, isRestore) {
          ...
          var arrayTw = new Array();
          var i = 0;
          var time = path.length < 3 ? 0.07 : 0.03; //Create list tween

          while (i < path.length) {
            var temp = path[i];
            var newPos = new Vec3(temp.col * (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            ...
            var tw = tween(this.currentFruitCell).to(time, {
              position: newPos
            }, {
              easing: 'smooth'
            });
          ...
          var _loop = function _loop(_i) {
            arrayTw[_i].call(() => arrayTw[_i + 1].start());
          };

          for (var _i = 0; _i < arrayTw.length - 1; _i++) {
            _loop(_i);
          } //Play tween move
        ...
        */
        ...
        moveComplete(path, isRestore) {
          ...
          if (!isRestore) {
            ...
            this.turn.prev = _extends({}, this.turn);
            var currentType = this.currentFruitCell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
              error: Error()
            }), Cell) : Cell).getTypeFruit();
            this.currentFruitCell = null; //Remove next cell if it is in path

            var nextCell = this.getNextCellByCoor(path[path.length - 1]);
            ...
            if (!this.deleteCells(path[path.length - 1], currentType)) {
              this.listNextCells.map(cell => {
                ...
                var coor = cellComponent.getCoordinates();
                var index = coor.row * (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
                  error: Error()
                }), Constants) : Constants).BOARD_SIZE + coor.col;
                var type = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
                  error: Error()
                }), Cell) : Cell).getTypeFruit();
        ...
        handleUndo() {
          ...
          var number = this.numberItem.get((_crd && ItemType === void 0 ? (_reportPossibleCrUseOfItemType({
            error: Error()
          ...
          this.turn = _extends({}, this.turn.prev);
        ...
        restoreNewCell(listCell) {
          listCell.map(cell => {
            var component = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
              error: Error()
            }), Cell) : Cell);
            ...
            var coor = component.getCoordinates();
        ...
        restoreCurrentCell(cell, path) {
          ...
          var currentCoor = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
          }), Cell) : Cell).getCoordinates();
          var oldCoor = path[path.length - 1];
          var type = cell.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
          }), Cell) : Cell).getTypeFruit();
        ...
        onTouchStartItem(event, type, board) {
          ...
          var location = this.gameView.getLocationTouch(event, this.nodeModal.boardView);
          var coor = this.getIndexByLocation(location);
        ...
        onTouchMoveBoardView(event) {
          if (!this.boardItem || this.isPause) return;
          var location = this.gameView.getLocationTouch(event, this.nodeModal.boardView);
          var coor = this.getIndexByLocation(location); //Get list location

          var listCoor = new Array();
          var temp = {
            row: 8 - coor.row,
            col: coor.col
          };
        ...
        onTouchEndBoardView() {
          ...
          var numberFruit = this.getListFruitCell().length;
          var listCells = this.getListDeleteCell(this.listCoorOfItem);
          if (this.currentItem === (_crd && ItemType === void 0 ? (_reportPossibleCrUseOfItemType({
            error: Error()
          ...
          setTimeout(() => {
            for (var item of listCells) {
              ...
              var coor = item.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
                error: Error()
              }), Cell) : Cell).getCoordinates();
              this.matrixManager.emptyCell(coor.row * (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
                error: Error()
            ...
            var number = this.numberItem.get(this.currentItem) - 1;
            this.listCoorOfItem = [];
        ...
        getIndexByLocation(location) {
          var row = Math.floor(location.y / (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          }), Constants) : Constants).CELL_SIZE);
          var col = Math.floor(location.x / (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          }), Constants) : Constants).CELL_SIZE);
          return {
            row: row,
        ...
        getNextCellByCoor(coor) {
          var cell = this.listNextCells.find(item => item.getComponent(_crd && Cell === void 0 ? (_reportPossibleCrUseOfCell({
            error: Error()
        ...
        removeNodeFromList(array, node) {
          var index = array.findIndex(item => item === node);
        ...
        generateListNextCells() {
          ...
          this.listNextTypes.map(type => {
            var cell = this.spawnCellFruit(type);
        ...
        deleteCells(coor, type) {
          var listCoor = this.matrixManager.getListDelete(coor, type);
          var listDeleteCell = this.getListDeleteCell(listCoor);
          ...
          if (listCoor.length > 0) {
            ...
            var score = listCoor.length * this.coefficient * this.combo;
        ...
        randomGift() {
          ...
          for (var i = 0; i < 55; i++) array.push(3);

          for (var _i2 = 0; _i2 < 40; _i2++) array.push(2);

          for (var _i3 = 0; _i3 < 5; _i3++) array.push(0);

          array = array.filter(item => item !== this.prevGift);
          var random = randomRangeInt(0, array.length);
          random = array[random];
          console.log(random);
          this.gameView.playAniamtionGift(random);

          if (random < 3) {
            var number = this.numberItem.get(random);
        ...
        getListFruitCell() {
          var listCell = this.nodeModal.boardFruit.children;
        ...
        checkGameOver() {
          if (this.matrixManager.arrayIndex.length <= 0) {
            ...
            var count = 0;
            var isCompleted = false; // let interval = setInterval(() => {
            //     count++;
...
//# sourceMappingURL=e4792899bc9b6cd19533cf5fab208071aaa33cf3.js.map